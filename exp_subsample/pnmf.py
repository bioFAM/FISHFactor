# Requires PNMF implementation from https://github.com/willtownes/nsf-paper
    ...
    for group in data.group.unique()[:n_cells]:
        ...
        for subsample_fraction in subsample_fractions:
            for bin_res in bin_res_list:
                if os.path.exists('results/pnmf/cell_%s/subsample_%s/binres_%s.pkl'
                    %(group, subsample_fraction, bin_res)):
                    print('File already exists: results/pnmf/cell_%s/subsample_%s/binres_%s.pkl'
                        % (group, subsample_fraction, bin_res))
                    continue
                ...
                if not os.path.exists('results/pnmf/cell_%s/subsample_%s/' %(group, subsample_fraction)):
                    os.makedirs('results/pnmf/cell_%s/subsample_%s/' %(group, subsample_fraction))

                pickle.dump(results, open('results/pnmf/cell_%s/subsample_%s/binres_%s.pkl' %(group, subsample_fraction, bin_res), 'wb'))   