# requires the NSF implementation from https://github.com/willtownes/nsf-paper
    ...
    for bin_res in bin_res_list:
        for factor_set in factor_set_list:
            for intensity_scale in intensity_scale_list:
                if os.path.exists('results/nsf_paper/factors_%s/intensity_%s/binres_%s.pkl'
                    %(factor_set, intensity_scale, bin_res)):
                    print('File already exists: results/nsf_paper/factors_%s/intensity_%s/binres_%s.pkl'
                    %(factor_set, intensity_scale, bin_res))
                    continue
                ...
                fit = pf.ProcessFactorization(J, L, Z, psd_kernel=ker,
                    nonneg=True, lik="poi")
                ...
                if not os.path.exists('results/nsf_paper/factors_%s/intensity_%s/'
                    %(factor_set, intensity_scale)):
                    os.makedirs('results/nsf_paper/factors_%s/intensity_%s/'
                    %(factor_set, intensity_scale))

                pickle.dump(results, open('results/nsf_paper/factors_%s/intensity_%s/binres_%s.pkl'
